...
function deleteTrack(unqfy, trackId) {
  return unqfy.deleteTrack(trackId);
}

function deleteArtist(unqfy, artistId) {
 
  return  unqfy.deleteArtist(artistId);
}

function deleteAlbum(unqfy,albumId){
  return unqfy.deleteAlbum(albumId);
}

function deletePlaylist(unqfy,playlistId){
  return unqfy.deletePlaylist(playlistId);
}

function getAllArtists(unqfy) {
 
  return  unqfy.getAllArtists();
}
...
describe('our Tests', () => {
  ...
  it('should delete a track to an album', () => {
    const artist = createAndAddArtist(unqfy, 'Guns n\' Roses', 'USA');
    const album = createAndAddAlbum(unqfy, artist.id, 'Appetite for Destruction', 1987);
    const track = createAndAddTrack(unqfy, album.id, 'Welcome to the jungle', 200, ['rock', 'hard rock']);
    assert.equal(track.name, 'Welcome to the jungle');
    ...
    assert.lengthOf(track.genres, 2);
    deleteTrack(unqfy, 1);
    assert.lengthOf(album.tracks, 0);
    assert.equal(album.tracks.includes(track), false);
    
    

  });

  it('should delete an artist from unqfy', () => {
    const artist = createAndAddArtist(unqfy, 'Guns n\' Roses', 'USA');
    assert.lengthOf(unqfy.getAllArtists(), 1);
    deleteArtist(unqfy, 1);
    assert.lengthOf(unqfy.getAllArtists(), 0); 

  });

  it('should delete an album from an artist', () => {
    const artist = createAndAddArtist(unqfy, 'Guns n\' Roses', 'USA');
    const album1 = createAndAddAlbum(unqfy, artist.id, 'Roses Album', 1987);
    assert.lengthOf(unqfy.getAllAlbums(), 1);
    deleteAlbum(unqfy, 1);
    assert.lengthOf(unqfy.getAllAlbums(), 0);
  });

  it('should delete a playlist from UNQfy', () => {
    unqfy.createPlaylist('my playlist', ['pop', 'rock'], 1400);
    assert.lengthOf(unqfy.playlists, 1);
    deletePlaylist(unqfy, 1);
    assert.lengthOf(unqfy.playlists, 0);
    
  });

  it('when you delete a Track it should delete that track from all the playlists of UNQfy', () => {
    const artist = createAndAddArtist(unqfy, 'Guns n\' Roses', 'USA');
    const album = createAndAddAlbum(unqfy, artist.id, 'Appetite for Destruction', 1987);
    const track = createAndAddTrack(unqfy, album.id, 'Welcome to the jungle', 200, ['rock', 'hard rock']);
    const playlist = unqfy.createPlaylist('my playlist', ['pop', 'rock'], 1400);
    assert.lengthOf(unqfy.getPlaylistById(1).tracks, 1);
    deleteTrack(unqfy, 1);
    assert.lengthOf(unqfy.getPlaylistById(1).tracks, 0);

  });

  it('when you delete an Album it should delete All albums tracks from all the playlists of UNQfy', () => {
    const artist = createAndAddArtist(unqfy, 'Guns n\' Roses', 'USA');
    const album = createAndAddAlbum(unqfy, artist.id, 'Appetite for Destruction', 1987);
    const track = createAndAddTrack(unqfy, album.id, 'Welcome to the jungle', 200, ['rock', 'hard rock']);
    const playlist = unqfy.createPlaylist('my playlist', ['pop', 'rock'], 1400);
  ...
  it('when you delete an Artist it should delete All artists tracks from all the playlists of UNQfy', () => {
    const artist = createAndAddArtist(unqfy, 'Guns n\' Roses', 'USA');
    const album = createAndAddAlbum(unqfy, artist.id, 'Appetite for Destruction', 1987);
    const track = createAndAddTrack(unqfy, album.id, 'Welcome to the jungle', 200, ['rock', 'hard rock']);
    const artist2 = createAndAddArtist(unqfy, 'Michael Jackson', 'USA');
    ...
    createAndAddTrack(unqfy, album3.id, 'Thriller', 200, ['pop', 'movie']);
    const playlist = unqfy.createPlaylist('my playlist', ['pop', 'rock'], 1400);
    assert.lengthOf(unqfy.getPlaylistById(1).tracks, 2);
    deleteArtist(unqfy, 1);
    assert.lengthOf(unqfy.getPlaylistById(1).tracks, 1);
